...
export default function SpotifyWidget() {
  ...
  const calculateCurrentTime = useCallback(() => {
    if (!spotifyData?.is_playing || !spotifyData.current_time)
      return spotifyData?.current_time || 0;
  ...
  useEffect(() => {
    ...
    window.addEventListener("scroll", updateBottomReleaseOffset, {
      passive: true,
    });
  ...
  const displayCurrentTime = spotifyData.is_playing
    ? currentTime
    : spotifyData.current_time || 0;
  const progressPercentage = spotifyData.duration
    ? Math.min((displayCurrentTime / spotifyData.duration) * 100, 100)
    : 0;
      ...
      >
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0 flex flex-col justify-center">
            ...
            <p className="text-xs text-text-muted truncate mb-0.5">
              {spotifyData.artist}
            </p>
